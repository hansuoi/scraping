...
# 6個目のpタグに武器ジャンル名が記載されている

# ソースコードの中からpタグを探し、その中身を配列に格納
para = soup.findAll("p")
#print(para)

# 6個目のpタグに書かれている内容だけぶっこ抜く
weapon_para = para[6]
#print(weapon_para)


# 6個目のpタグの中にあるaタグに、各武器ジャンルへのURLが書かれている

# 6個目のpタグの中にあるaタグの中身を配列に格納
...
for urlName in urls:
    ...
    for i in range(1, len(guns)):
        ...
        # となっており、今回は、
        # [1名前, 2威力, 3射程, 4反動, 5連射速度, 7弾数]
        # だけゲットする。
        # stringは、タグに挟まれていた文章だけ抜き出すやつ
        data = [gun[1].string, gun[2].string, gun[3].string, gun[4].string, gun[5].string, gun[7].string]
...
datalist = pd.DataFrame(datalist, columns=['名前','威力','射程','反動','連射速度','弾数'])
print(datalist)